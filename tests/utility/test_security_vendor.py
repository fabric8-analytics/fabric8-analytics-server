...
from bayesian.utility.security_vendor import VendorAnalyses, ResponseBuilder, \
    convert_version_to_proper_semantic, version_info_tuple
...
class AdequacyTest(unittest.TestCase):
    ...
    def test_version_info_tuple(self):
        ...
        version_obj = convert_version_to_proper_semantic(version_str, package_name)
        version_info = version_info_tuple(version_obj)
        self.assertEqual(len(version_info), 4)
        self.assertEqual(version_info[0], version_obj.major)
        self.assertEqual(version_info[1], version_obj.minor)
        self.assertEqual(version_info[2], version_obj.patch)
        self.assertEqual(version_info[3], version_obj.build)

    def test_version_info_tuple_negative(self):
        """Test function for convert version to tuple."""
        version_info = version_info_tuple('version')
        self.assertEqual(len(version_info), 4)

    def test_convert_version_to_proper_semantic(self):
        """Test function for convert version to proper semantic."""
        version_obj = convert_version_to_proper_semantic("", 'package')
        self.assertEqual(0, version_obj.major)
        self.assertEqual(0, version_obj.minor)
        self.assertEqual(0, version_obj.patch)

    def test_convert_version_to_proper_semantic_exception(self):
        """Test function for convert version to proper semantic throw exception."""
        version_obj = convert_version_to_proper_semantic("THROW")
        self.assertEqual(0, version_obj.major)
        self.assertEqual(0, version_obj.minor)
        self.assertEqual(0, version_obj.patch)
...
class VendorAnalysesTest(unittest.TestCase):
    ...
    @staticmethod
    @patch('bayesian.utility.db_gateway.GraphAnalyses.get_data_from_graph', return_value=None)
    ...
    @staticmethod
    @patch('bayesian.utility.security_vendor.ResponseBuilder.generate_recommendation')
    @patch('bayesian.utility.security_vendor.VendorAnalyses.is_package_known', return_value=True)
    @patch('bayesian.utility.db_gateway.GraphAnalyses.get_data_from_graph')
    ...
    @staticmethod
    @patch('bayesian.utility.db_gateway.GraphAnalyses.get_data_from_graph', return_value=Exception)
...
class ResponseBuilderTest(unittest.TestCase):
    ...
    @patch('bayesian.utility.security_vendor.ResponseBuilder.generate_response', return_value={})
    @patch('bayesian.utility.security_vendor.ResponseBuilder.get_severity')
    @patch('bayesian.utility.security_vendor.ResponseBuilder.get_vulnerabilities_count')
    @patch('bayesian.utility.security_vendor.ResponseBuilder.get_version_without_cves')
    @patch('bayesian.utility.security_vendor.ResponseBuilder.get_cve_maps', return_value=[])
    @patch('bayesian.utility.security_vendor.ResponseBuilder.has_cves', return_value=True)
    ...
    def test_get_message_with_pvt_vul(self):
        ...
        response_obj = ResponseBuilder(self.eco, self.pkg, self.ver)
        response_obj.pvt_vul = 1
        response_obj.severity = ['high']
        message = response_obj.get_message()
        self.assertIsInstance(message, str)

    def test_get_message_without_pvt_vul(self):
        """Test Message without Private Vulnerability."""
        response_obj = ResponseBuilder(self.eco, self.pkg, self.ver)
    ...
    @patch('bayesian.utility.security_vendor.ResponseBuilder.get_cve_maps')
    @patch('bayesian.utility.security_vendor.ResponseBuilder.get_registration_link')
    @patch('bayesian.utility.security_vendor.ResponseBuilder.get_message')
