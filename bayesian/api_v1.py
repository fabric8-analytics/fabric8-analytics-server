...
class StackAnalyzerGET(ResourceWithSchema):
    method_decorators = [login_required]
    # schema_ref = SchemaRef('stack_analyses', '2-1-4')
    ...
    def get(external_request_id):
        ...
        analyzer = retrieve_worker_result(rdb, external_request_id, "StackAnalyzerTask")
        if analyzer is not None and 'task_result' in analyzer:
            if analyzer['task_result'] is None:
                raise HTTPError(500, 'Invalid manifest file(s) received. '
                                     'Please submit valid manifest files for stack analysis')
...
add_resource_no_matter_slashes(StackAnalyzerGET, '/stack-analyzer/<external_request_id>')
