...
exec mod_wsgi-express start-server --entry-point bayesian --application-type module --callable-object app --port 5000 --user coreapi --group coreapi --processes 4 --threads 10 ${!F8A_DEBUG:---reload-on-changes} --access-log --access-log-format "%h %l %u %t \"%r\" %>s %b %{Referer}i \"%{User-agent}i\"" --log-to-terminal --python-eggs /home/coreapi --include-file /etc/httpd/conf.d/coreapi-httpd.conf --modules-directory /usr/lib64/httpd/modules --header-buffer-size 65500 
